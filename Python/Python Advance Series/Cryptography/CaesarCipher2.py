def encrypt(text, key):
    ...
    for char in text:
        if char.isalpha():  # Check if the character is a letter
            ...
            shift = ord('X') if char.islower() else ord('X') - 32
            # Encrypt the character using the custom alphabet series and the key
...
def main():
    ...
    if choice == 1:  # If the user chose encryption
        encrypted_text = encrypt(text, key)  # Encrypt the text using the custom alphabet and the key
        print(f"Encrypted text: {encrypted_text}")  # Display the encrypted text
    elif choice == 2:  # If the user chose decryption
        decrypted_text = decrypt(text, key)  # Decrypt the text using the custom alphabet and the key
